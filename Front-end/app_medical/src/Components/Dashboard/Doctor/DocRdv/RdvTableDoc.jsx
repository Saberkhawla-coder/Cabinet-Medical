...
export default function RdvTableDoc() {
  ...
  return (
    <div className="mt-3 bg-white shadow-xl rounded-2xl  overflow-hidden">
       <div className="p-4 flex justify-between items-center">
        <h2 className="text-lg font-bold text-gray-700">Mes Rendez-vous</h2>
  ...
  <thead >
    <tr className="bg-gray-100/30 text-gray-700 text-sm uppercase tracking-wide ">
      ...
      <th className="py-3 px-6 text-center font-semibold">Heure</th>
      <th className="py-3 px-6 text-center font-semibold">Statut</th>
    </tr>
  </thead>
  ...
  <tbody className="divide-y text-center">
    {currentMyAppointments.length === 0 ? (
      ...
      currentMyAppointments.map((appoint) => (
        ...
        >
          <td className="px-6 py-4 text-center font-medium text-gray-800">{appoint?.patient?.user?.name || "—"}</td>
          ...
          <td className="px-6 py-4 text-gray-700 font-medium">{appoint?.appointment_time || "—"}</td>
          ...
          <td className="px-6 py-4 flex gap-2 justify-center ">
            {["confirmed", "pending", "cancelled"].map((st) => (
              ...
              >
                {st === "confirmed" ? "Confirmer" : st === "pending" ? "En attente" : "Annuler"}
              </span>
            ))}
          </td>
        </tr>
